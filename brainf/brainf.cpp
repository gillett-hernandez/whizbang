...
public:
    ...
    int run() {
        ...
        TokenType token = this->tokens[ti];
        for(;;) {
            ...
            switch(token) {
                ...
                case (IN):{
                    ...
                    ii++;
                    debug("incremented input pointer");
                    break;
                }
                case (JZ):{
                    debug(repr(JZ));
...
private:
    ...
    TokenType consume(int& i) {
        ...
        i++; // increment token count and return token at new count
    ...
    int jumpforward(int& i){
        ...
        i--; // move back so that tokens[i] matches JNZ
    ...
    int jumpback(int& i){
        ...
        i++; // to move instruction pointer to match the bracket
...
std::string brainCluck(std::string code, std::string input) {
...
int main(int argc, char const *argv[]) {
    if (argc == 1) {
        ...
        auto actual1 = brainCluck(",>,<[>[->+>+<<]>>[-<<+>>]<<<-]>>.",dw);
        std::cout << static_cast<int>(actual1[0]) << std::endl;

        auto actual2 = brainCluck("++++++++++[>+++++++>++++++++++>+++>+<<<<-]>++.>+.+++++++..+++.>++.<<+++++++++++++++.>.+++.------.--------.>+.", "");
